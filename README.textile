...
* With an empty workspace the tags are calculated after we made a checkout. This may take a long time. A better approach needs a new feature in the git-client-plugin, which is planned.
...
* Add a new method listRemoteTags(URL) to git-client-plugin to use. Will speed up listing tags when workspace is empty and cloning takes a long time.
